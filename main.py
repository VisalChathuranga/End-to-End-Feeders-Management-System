...
DIRECTORY_PATH = os.path.join(base_path, 'Feeders')

# File paths for storing machine types and serial numbers
MACHINE_TYPES_FILE = os.path.join(base_path, 'machine_types.json')
SERIAL_NUMBERS_FILE =  os.path.join(base_path,'serial_numbers.json')
...
def get_file_path(machine_type):
    if machine_type not in ["Siplace", "Yamaha"] + new_machines:
        ...
        return None
    return os.path.join(base_path, 'Feeders', machine_type, f'{machine_type} Feeder Inventory.xlsx')
...
def open_repair_window():
    def submit_repair():
        ...
        repaired_by = user_type_combo.get()
        ...
        if not date or not feeder_type or not serial_number or not fault or not repair_history or not repaired_by:
            ...
            try:
                ...
                ws.cell(row=next_row, column=7, value=repaired_by)
                wb.save(file_path)
...
def open_maintenance_window():
    ...
    maintenance_window.attributes("-topmost", 0)  # Ensure the window is on top
...
def show_reminders(frame):
    file_path = os.path.join(base_path, 'Feeders', 'Annual Maintenance', 'Annual Maintenance.xlsx')
    ...
    try:
        ...
        if reminders:
            for row_idx, (machine_type, serial_number, next_date, remaining_days) in enumerate(reminders, start=1):
                color = "red" if remaining_days < 0 else "White"  # Red if overdue

                # Create labels for each cell with color coding for overdue maintenance
                ctk.CTkLabel(table_frame, text=machine_type, font=("Arial", 12), anchor="center", text_color=color).grid(row=row_idx, column=0, padx=5, pady=5, sticky="ew")
                ctk.CTkLabel(table_frame, text=serial_number, font=("Arial", 12), anchor="center", text_color=color).grid(row=row_idx, column=1, padx=5, pady=5, sticky="ew")
                ctk.CTkLabel(table_frame, text=next_date.strftime('%Y-%m-%d'), font=("Arial", 12), anchor="center", text_color=color).grid(row=row_idx, column=2, padx=5, pady=5, sticky="ew")
                ctk.CTkLabel(table_frame, text=str(remaining_days), font=("Arial", 12), anchor="center", text_color=color).grid(row=row_idx, column=3, padx=5, pady=5, sticky="ew")
                # Add Reminder Date column
                ctk.CTkLabel(table_frame, text=next_date.strftime('%Y-%m-%d'), font=("Arial", 12), anchor="center", text_color=color).grid(row=row_idx, column=4, padx=5, pady=5, sticky="ew")
...
class SiplaceMaintenanceWindow:
    def __init__(self, root, machine_type):
        ...
        self.siplace_window.geometry('524x800')
        self.siplace_window.resizable(True, True)  # Allow resizing
        self.siplace_window.attributes("-topmost", 0)
    ...
    def create_widgets(self):
        ...
        # Assuming your images are in the 'Images/Siplace Images' folder relative to the script location
        image_folder = os.path.join(os.path.dirname(__file__), 'Images', 'Siplace Images')

        # List of images with updated relative paths
        image_paths = [
        os.path.join(image_folder, "1.Cleaning the Pickup Area.png"),
        os.path.join(image_folder, "2.Checking the Centering Pin, EDIF and Roller.png"),
        os.path.join(image_folder, "3.Checking the Cover Tape.png"),
        os.path.join(image_folder, "4.Checking the Foil Draw-In.png"),
        os.path.join(image_folder, "5.Checking the Slide Bearings.png"),
        os.path.join(image_folder, "6.Cleaning the Side Covers.png")
        ]
    ...
    def submit_maintenance(self):
        ...
        if not date or not feeder_type or not serial_number or not mainteince_by:
            messagebox.showerror("Error", "Please fill all the required fields.")
            return

        # Validate date format
        try:
            date_obj = datetime.strptime(date, "%Y-%m-%d")  # Ensure this format matches your input
        except ValueError:
        ...
        if not all(var.get() for var in self.check_vars):
            messagebox.showerror("Error", "Please ensure all checkboxes are ticked.")
            return
        ...
        try:
            ...
            if row_to_update:
                ...
                ws.cell(row=row_to_update, column=4, value=remarks if remarks else "OK")
                ...
                ws.cell(row=next_row, column=2, value=feeder_type)
                ws.cell(row=next_row, column=3, value=serial_number)
                ws.cell(row=next_row, column=4, value=remarks if remarks else "OK")
...
class YamahaMaintenanceWindow:
    def __init__(self, root, machine_type):
        ...
        self.yamaha_window.attributes("-topmost", 0)
    ...
    def create_widgets(self):
        ...
        # Assuming your images are in the 'Images/Siplace Images' folder relative to the script location
        image_folder = os.path.join(os.path.dirname(__file__), 'Images', 'Yamaha Images')


        # List of images with updated relative paths
        image_paths = [
            os.path.join(image_folder, "1.Check the tape guide assembly and tape guide front lever.png"),
            os.path.join(image_folder, "2.Check front block.png"),
            os.path.join(image_folder, "3.Check Board connector.png"),
            os.path.join(image_folder, "4.Check Under Rail.png")
        ]
...
def open_admin_dashboard():
    ...
    def add_machine():
        def submit_new_machine():
            ...
            folder_path = os.path.join(base_path, 'Feeders', machine_name)
        ...
        frame = ctk.CTkFrame(add_machine_window)
    ...
    def remove_machine():
        ...
        # Create and style the frame using customtkinter
        frame = ctk.CTkFrame(remove_machine_window)
        frame.pack(padx=15, pady=15, fill='both', expand=True)
    ...
    admin_dashboard.attributes("-topmost", 0)  # Ensure the window is on top
...
# USER Tab
user_frame = ctk.CTkFrame(user_tab)  
...
users = ["Kasun", "Ashen"]
global user_type_combo
user_type_combo = ttk.Combobox(user_frame, values=users, width=30)
user_type_combo.pack(pady=10)
...
repair_button = ctk.CTkButton(user_frame, text="Repair", command=open_repair_window,fg_color="#1E90FF",hover_color="#104E8B", text_color="white",height=45)
repair_button.pack(side=tk.LEFT, padx=20)

maintenance_button = ctk.CTkButton(user_frame, text="Maintenance", command=open_maintenance_window,fg_color="#1E90FF",hover_color="#104E8B", text_color="white",height=45)
